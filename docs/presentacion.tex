...
% Superficie de error más realista
\draw[thick, blue] plot[smooth, domain=-3:3] (\x, {0.2*\x*\x*\x*\x - 0.8*\x*\x + 2});
\draw[->] (-3.5,0) -- (3.5,0) node[right] {Parámetros $\theta$};
\draw[->] (0,0) -- (0,4) node[above] {Error $J(\theta)$};

% Múltiples puntos de descenso
\filldraw[red] (-2,2.8) circle (2pt) node[above] {\small Inicio};
\draw[->, thick, red] (-2,2.8) -- (-1.5,2.1);
\draw[->, thick, red] (-1.5,2.1) -- (-1,1.6);
\draw[->, thick, red] (-1,1.6) -- (-0.5,1.4);
\filldraw[green] (-0.5,1.4) circle (2pt) node[below] {\small Meta};

% Gradiente
\draw[->, thick, orange] (-2,2.8) -- (-1.3,2.5) node[midway, above] {$-\nabla J$};
\end{tikzpicture}
\end{center}
\end{column}
\begin{column}{0.5\textwidth}
\textbf{Imagina un montañista perdido en la niebla:}
\begin{itemize}
\item Solo puede ver el \textcolor{red}{\textbf{terreno local}}
\item Quiere llegar al \textcolor{verdeClaro}{\textbf{valle más bajo}}
\item Estrategia: seguir la \textcolor{orange}{\textbf{pendiente más empinada}} hacia abajo
\end{itemize}
...
\item \textbf{Posición} = parámetros del modelo
